...
public class ChooseTrostPrizeEvent extends MessageHandler {
    private static final Logger LOGGER = Logger.getLogger(ChooseTrostPrizeEvent.class.getName());
    ...
    public void handle() throws Exception {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection()) {
            ...
            try (PreparedStatement checkStmt = connection.prepareStatement(checkQuery)) {
                ...
                try (ResultSet resultSet = checkStmt.executeQuery()) {
                    if (resultSet.next() && resultSet.getInt("is_win") == 1) {
                        if (prizeId == 1) {
                            Emulator.getGameEnvironment().getHabboManager().getHabboInfo(userId).getHabboStats().achievementScore += 50;
                        }
            ...
            LOGGER.severe("ChooseTrostPrizeEvent sırasında bir hata oluştu: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
