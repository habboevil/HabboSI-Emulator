...
public class ChoosePremiumPrizeEvent extends MessageHandler {
    private static final Logger LOGGER = Logger.getLogger(ChoosePremiumPrizeEvent.class.getName());
            ...
            try (PreparedStatement checkStmt = connection.prepareStatement(checkQuery)) {
                ...
                try (ResultSet resultSet = checkStmt.executeQuery()) {
                    if (resultSet.next() && resultSet.getInt("is_win") == 1) {
                        if (prizeId == 1) {
                            Emulator.getGameEnvironment().getHabboManager().giveCredits(userId, 2000);
                            Habbo habbo = Emulator.getGameEnvironment().getHabboManager().getHabbo(userId);
                            if (habbo != null) {
                                habbo.givePoints(5, 20);
                            }
                        } else {
                            Habbo habbo = Emulator.getGameEnvironment().getHabboManager().getHabbo(userId);
                            if (habbo != null) {
                                habbo.givePoints(103, 4);
                            }
                        }
