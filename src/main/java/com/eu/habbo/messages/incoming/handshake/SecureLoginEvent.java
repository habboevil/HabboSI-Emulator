        ...
        if (this.client.getHabbo() == null) {
            ...
            if (habbo != null) {
                ...
                try (PreparedStatement checkStatement = connection.prepareStatement(
                        "SELECT last_login_daily_prize FROM users WHERE id = ?")) {

                    checkStatement.setInt(1, this.client.getHabbo().getHabboInfo().getId());
                    ResultSet resultSet = checkStatement.executeQuery();

                    if (resultSet.next()) {
                        long lastLoginDailyPrize = resultSet.getLong("last_login_daily_prize");

                        if (lastLoginDailyPrize == 0) {
                            habbo.getHabboInfo().addCredits(1000);
                            habbo.getHabboInfo().addPixels(10);
                            String data = "dailyPrizeView/show";
                            habbo.getClient().sendResponse((new NuxAlertComposer(data)).compose());
                            LOGGER.info("User ID " + this.client.getHabbo().getHabboInfo().getId()
                                    + " received daily prize for the first time.");

                            try (PreparedStatement updateStatement = connection.prepareStatement(
                                    "UPDATE users SET last_login_daily_prize = ? WHERE id = ?")) {
                                updateStatement.setLong(1, Instant.now().getEpochSecond());
                                updateStatement.setInt(2, this.client.getHabbo().getHabboInfo().getId());
                                updateStatement.executeUpdate();
                            }
                        } else {
                            long lastLoginTimeEpoch = lastLoginDailyPrize;
                            if (Instant.ofEpochSecond(lastLoginTimeEpoch).plusSeconds(86400).isBefore(Instant.now())) {
                                habbo.getHabboInfo().addCredits(1000);
                                habbo.getHabboInfo().addPixels(10);
                                String data = "dailyPrizeView/show";
                                habbo.getClient().sendResponse((new NuxAlertComposer(data)).compose());
                                LOGGER.info("User ID " + this.client.getHabbo().getHabboInfo().getId()
                                        + " received daily prize.");
                                ...
                                LOGGER.info("User ID " + this.client.getHabbo().getHabboInfo().getId()
                                        + " is not eligible for daily prize.");
                            }
                        }
                    }
                }  catch (SQLException e) {
                LOGGER.error("Error processing daily prize for user ID " + this.client.getHabbo().getHabboInfo().getId(), e);
                 }
