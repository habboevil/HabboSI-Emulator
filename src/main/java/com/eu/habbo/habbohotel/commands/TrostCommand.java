...
public class TrostCommand extends Command {
    public TrostCommand() {
        super("cmd_trost", Emulator.getTexts().getValue("commands.keys.cmd_trost").split(";"));
    }

    private static final Logger LOGGER = LoggerFactory.getLogger(TrostCommand.class);
        ...
        if (gameClient.getHabbo().getHabboInfo().getCurrentRoom() != null ) {
            if (params.length == 2) {
                ...
                if (habbo == null) {
                    ...
                    String data = "ChooseTrostPrizeView/show";
                    ...
                    try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                        ...
                        if (rowsAffected > 0) {
                            LOGGER.info("Successfully marked " + habbo.getHabboInfo().getUsername() + " as a Trost winner!");
                        } else {
                            LOGGER.warn("Failed to update winner status for " + habbo.getHabboInfo().getUsername());
                        }
                    } catch (Exception e) {
                        LOGGER.error("Error processing Trost Command for " + habbo.getHabboInfo().getUsername(), e);
                    }
